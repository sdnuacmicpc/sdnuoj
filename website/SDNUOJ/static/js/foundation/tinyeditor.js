...
TINY.editor = function() {
    ...
    var controls = ['bold', 'italic', 'underline', 'strikethrough', 'subscript', 'superscript', '|', 'leftalign', 'centeralign', 'rightalign', 'blockjustify', '|', 'orderedlist', 'unorderedlist', '|', 'outdent', 'indent', '|', 'unformat', 'n', 'font', 'size', 'style', '|', 'undo', 'redo', '|', 'image', 'hr', 'link', 'unlink'];
    var toggles = { text: 'Source Code', activetext: 'Design Mode', cssclass: 'toggle' };
    function edit(n, obj) {
        this.n = n;
        window[n] = this;
        this.t = T$(obj.id);
        ...
        l = obj.controls ? obj.controls.length : controls.length,
        i = 0;
        this.i = document.createElement('iframe');
        this.i.width = obj.width || '530';
        this.i.height = obj.height || '170';
        this.ie = T$$$();
        h.className = obj.rowclass || 'tinyeditor-header';
        p.className = obj.cssclass || 'tinyeditor';
        p.style.width = this.i.width.toString().indexOf("%") > -1 ? this.i.width : this.i.width + 'px';
        p.appendChild(h);
        for (i; i < l; i++) {
            var id = obj.controls ? obj.controls[i] : controls[i];
            if (id == 'n') {
                h = document.createElement('div');
                h.className = obj.rowclass || 'tinyeditor-header';
                p.appendChild(h)
                ...
                fonts = obj.fonts || ['Arial', 'Consolas', 'Courier New', 'Georgia', 'Times New Roman', 'Verdana', 'SimSun'],
                fl = fonts.length,
                x = 0;
        ...
        this.t.style.width = this.i.width.toString().indexOf("%") > -1 ? this.i.width : this.i.width + 'px';
        w.appendChild(this.t);
        w.appendChild(this.i);
        p.appendChild(w);
        this.t.style.display = 'none';
        if (obj.footer == null ? true : obj.footer) {
            var f = document.createElement('div');
            f.className = obj.footerclass || 'tinyeditor-footer';
            if (obj.toggle ? obj.toggle : toggles) {
                var to = obj.toggle ? obj.toggle : toggles,
                ts = document.createElement('div');
                ts.className = to.cssclass || 'toggle';
                ts.innerHTML = obj.toggletext || 'Source Code';
                ts.onclick = new Function(this.n + '.toggle(0, this);return false');
                f.appendChild(ts)
            }
            if (obj.resize == null ? true : obj.resize) {
                var ro = obj.resize,
                rs = document.createElement('div');
                rs.className = ro == null ? 'resize' : ro.cssclass;
        ...
        this.e.open();
        var m = '<html><head>',
        bodyid = obj.bodyid != null ? " id=\"" + obj.bodyid + "\"" : " id=\"editorbox\"";
        if (obj.cssfile) {
            for (var j = 0; j < obj.cssfile.length; j++) {
                m += '<link rel="stylesheet" href="' + obj.cssfile[j] + '" />'
            }
        }
        if (obj.css) {
    ...
    edit.prototype.toggle = function(post, div) {
        if (!this.d) {
            var v = this.t.value;
            if (div) {
                div.innerHTML = this.obj.toggletext || 'Source Code'
            }
            ...
            if (div) {
                div.innerHTML = this.obj.toggletext || 'Design Mode'
            }
            this.t.value = v;
            if (!post) {
                this.t.style.width = this.i.width + (this.i.width.indexOf('px') < 0 && this.i.width.indexOf('%') < 0 ? 'px' : '');
                this.t.style.height = this.i.height + (this.i.height.indexOf('px') < 0 && this.i.height.indexOf('%') < 0 ? 'px' : '');
